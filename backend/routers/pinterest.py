...
async def _pinterest_get(path: str, token: str, params: dict = {}) -> dict:
    """Helper — GET from Pinterest API with auth header."""
...
async def get_board_pins(
    ...
    Supports cursor-based pagination — pass `cursor` from the previous response.
    """
    ...
    pins = [
        ...
        if _best_image(p)  # skip pins with no usable image
    ]
