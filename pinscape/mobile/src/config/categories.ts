...
// The valid set of analysis categories — kept in sync with
// backend/models/schemas.py's CategoryKey enum.
export type CategoryKey = 'bedroom' | 'nails' | 'outfit' | 'hair' | 'makeup' | 'living';
...
// Colours reference the shared theme palette so that a single change in
// theme.ts propagates everywhere, rather than requiring hex updates in both
// files (which previously diverged silently if the theme changed).
const CATEGORIES: Record<CategoryKey, CategoryConfig> = {
  bedroom: {
    key: 'bedroom', label: 'Bedroom', icon: 'bed-king-outline',
    accentColor: Colors.purple400, bgColor: Colors.purple50,
  ...
  nails: {
    ...
    accentColor: Colors.pink400, bgColor: Colors.pink50,
  ...
  outfit: {
    ...
    accentColor: Colors.teal400, bgColor: Colors.teal50,
  ...
  hair: {
    ...
    accentColor: Colors.amber400, bgColor: Colors.amber50,
  ...
  makeup: {
    ...
    accentColor: Colors.coral400, bgColor: Colors.coral50,
