...
export default function BoardPinsScreen() {
  ...
  const confirm = () => {
    ...
    // Use the ref-based callback that PinsScreen registered on global so we
    // can return selected URLs without passing a function through React
    // Navigation params (which causes non-serialisable-value warnings and
    // breaks deep-link / state-persistence scenarios).
    //
    // The ref is stored per-boardId so that selections from multiple boards
    // accumulate correctly in PinsScreen's boardPinsByBoard map rather than
    // the last board's selection silently replacing earlier ones.
    const cb = (global as any).__pinscapeOnBoardPinsSelected?.current;
    if (cb) cb(params.boardId, urls);
        ...
        {selected.size > 0
          ? (
            <TouchableOpacity onPress={confirm} style={s.doneBtn}>
              <Text style={s.doneTxt}>Add {selected.size}</Text>
            </TouchableOpacity>
          )
          : <View style={{ width: 60 }} />}
      </View>
