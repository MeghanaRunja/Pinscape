...
export default function UploadScreen() {
  ...
  const pickPhoto = async (useCamera: boolean) => {
    ...
    const result = useCamera
      ...
      : await ImagePicker.launchImageLibraryAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Images, allowsMultipleSelection: true, quality: 0.85 });
    if (!result.canceled) setPhotos(p => [...p, ...result.assets]);
  };

  const showPicker = () => Alert.alert('Add photo', '', [
    { text: 'Camera', onPress: () => pickPhoto(true) },
    { text: 'Photo library', onPress: () => pickPhoto(false) },
    { text: 'Cancel', style: 'cancel' },
  ]);

  const toggleAngle = (a: string) => setAngles(prev => { const s = new Set(prev); s.has(a) ? s.delete(a) : s.add(a); return s; });

  const next = async () => {
    setUploading(true);
    try {
      const files = photos.map(p => ({ uri: p.uri, name: p.fileName || 'photo.jpg', type: p.mimeType || 'image/jpeg' }));
      const { data } = await uploadPhotos(files, params.categoryKey);
      nav.navigate('Pins', { categoryKey: params.categoryKey, photoKeys: data.keys, anglesCovered: [...angles] });
    } catch {
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity style={[s.zone, photos.length > 0 && s.zoneDone]} onPress={showPicker} activeOpacity={0.8}>
          <MaterialCommunityIcons name={photos.length > 0 ? 'check-circle-outline' : 'camera-outline'} size={34} color={photos.length > 0 ? Colors.teal400 : Colors.gray400} />
          <Text style={[s.zoneLbl, photos.length > 0 && { color: Colors.teal600 }]}>{photos.length > 0 ? `${photos.length} photo${photos.length > 1 ? 's' : ''} added` : 'Tap to upload or take a photo'}</Text>
          <Text style={s.zoneHnt}>{photos.length > 0 ? 'Tap to add more' : 'JPG, PNG, HEIC'}</Text>
        </TouchableOpacity>

        {photos.length > 0 && (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 10 }} contentContainerStyle={{ gap: 8, paddingHorizontal: Spacing.lg }}>
            {photos.map((p, i) => (
              <View key={i} style={s.thumb}>
                <Image source={{ uri: p.uri }} style={s.thumbImg} />
                <TouchableOpacity style={s.thumbX} onPress={() => setPhotos(ph => ph.filter((_, j) => j !== i))}>
        ...
        <View style={s.chipRow}>
          {cat.angles.map(a => (
            <TouchableOpacity key={a} style={[s.chip, angles.has(a) && s.chipSel]} onPress={() => toggleAngle(a)}>
              <MaterialCommunityIcons name={angles.has(a) ? 'check' : 'camera-outline'} size={12} color={angles.has(a) ? Colors.pink800 : Colors.gray400} />
              <Text style={[s.chipTxt, angles.has(a) && s.chipTxtSel]}>{a}</Text>
            </TouchableOpacity>
          ))}
        </View>

        <TouchableOpacity style={[s.btn, (photos.length === 0 || uploading) && s.btnOff]} onPress={next} disabled={photos.length === 0 || uploading} activeOpacity={0.85}>
